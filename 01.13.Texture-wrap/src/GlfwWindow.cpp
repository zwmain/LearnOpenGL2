...
void MainWindow::PrepareTexture()
{
    ...
    auto* imgPtr = stbi_load("assets/image/gugugaga.jpeg", &width, &height, &channels, STBI_rgb_alpha);
    ...
    //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // 设置纹理环绕方式：拉伸边缘像素
    //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); // 设置纹理环绕方式：拉伸边缘像素
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER); // 设置纹理环绕方式：设置预设的颜色
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER); // 设置纹理环绕方式：设置预设的颜色
    float borderColor[] = { 0.60f, 0.80f, 0.65f, 1.00f, 1.0f }; // 设置为一个颜色
    glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor); // 与GL_CLAMP_TO_BORDER配合，设置边界颜色
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // 设置纹理缩小过滤：线性过滤
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // 设置纹理放大过滤：线性过滤
